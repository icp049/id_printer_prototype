...
class BarcodePrinterApp:
    def __init__(self, root):
        ...
        ctk.CTkLabel(input_frame, text="Library Account Number:").grid(row=0, column=0, padx=10, pady=10)
    ...
    def create_print_mode_selector(self, parent):
        ...
        single_img = Image.open("snip1.PNG").resize((button_width, button_height))
        triple_img = Image.open("snip2.PNG").resize((button_width, button_height))
        ...
        for mode in ["Single Card", "Triple Keychain"]:
            sub_frame = ctk.CTkFrame(self.mode_frame, fg_color="transparent")
            sub_frame.pack(side="left", padx=20)
            ...
            label = ctk.CTkLabel(sub_frame, text=mode.capitalize(), font=("Arial", 14))
            label.pack(pady=(5, 0))

            self.mode_buttons[mode] = btn

        self.select_print_mode("single")

    def select_print_mode(self, mode):
        self.print_mode.set(mode)
